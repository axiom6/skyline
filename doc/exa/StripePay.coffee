...
class Pay
  ...
  formatCardNumber:(e) ->
    ...
    if re.test(value)
      e.preventDefault()
      @setPayTimeout -> $target.val(value + ' ' + digit)
  ...
  formatBackCardNumber:(e) ->
    ...
    if /\d\s$/.test(value)
      e.preventDefault()
      @setPayTimeout -> $target.val(value.replace(/\d\s$/, ''))
    # Remove digit if ends in space + digit
    else if /\s\d?$/.test(value)
      e.preventDefault()
      @setPayTimeout -> $target.val(value.replace(/\d$/, ''))
  ...
  formatExpiry:(e) ->
    ...
    if /^\d$/.test(val) and val not in ['0', '1']
      e.preventDefault()
      @setPayTimeout -> $target.val("0#{val} / ")
  ...
  formatBackExpiry:(e) ->
    ...
    if /\d\s\/\s$/.test(value)
      e.preventDefault()
      @setPayTimeout -> $target.val(value.replace(/\d\s\/\s$/, ''))
